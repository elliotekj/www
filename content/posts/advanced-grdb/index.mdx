...
requirement that projects can have a user-defined accent color and banner image
that are modelled into a `ProjectBrand` struct.
...
With all that in place, here's what a `Project` would look like in your app:
...
There's no comment section on here but I cross-post to dev.to which does have
one. If you have any remaining questions, feel free to post there.
