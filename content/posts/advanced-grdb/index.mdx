...
adopt GRDB's protocols so a struct can be saved to the database, and latsly we
looked at some basic querying.

In part 2, we have a look at how to observe and react to changes in the
...
NOTES:
- Do this over the observation so the whole "tasks belonging to a project" thing makes sense.

### Observing and reacting to changes

GRDB leverages SQLite's data change notifications to provide us with an
efficient `ValueObservation` tool that will in turn call either an `onChange`
or `onError` callback in our app. It's one of my favourite features of GRDB.

In part 1, a `Project` model was used for all of the examples. Here, we'll
pretend that a `Project` can have many tasks, and that we want to observe any
changes that occur to the tasks that belong to the project with an `id` of `1`,
be that the addition of a new task, a change to an existing one, etc.

To start, import GRDB and define an optional `TransactionObserver` property in
your controller. This where our observer will be kept in memory, and is the
type returned when `start` is called on a `ValueObservation`.
...
Our `tasksObserver` now needs to be defined. We'll break its configuration out
into its own function; remember to call it in `viewDidLoad` or wherever you're
handling view setup.

When `start` is first called on the `ValueObservation`, it will run the query
it's observing and return the result to the `onChange` callback. This is nice
because it means we don't have to duplicate the query in our controller (one
query for initial setup, then one for the observer). We can just start the
observer when the view loads and `onChange` will receive all of the tasks that
match the request.
...
NOTES:
- Remember to remove the observer on viewWillClose or whatever it is
- On start will make the query once so you don't need to do the query then start the observer.
- Recommend the DeepDiff pod.
