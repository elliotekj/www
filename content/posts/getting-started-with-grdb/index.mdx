...
We now have a database and a connection to it. All that's left to do before we
can populate it is to create a table and define its columns.
...
way to evolve the structure of the database over time. **They are guaranteed to
be applied in order, and only once.** What that means for us is that when
a user updates your app and you have, for example, added a new feature that
required a new column in one of your tables, the migration that added that
column will be the only one that's run.
...
remember that once the first version version of your app has been released, you
can't go back and make changes to old migrations because those changes won't be
applied on user's devices. You can make changes, but you need to do so in a new
migration.
...
method to actually run the migrations.
...
### Part 2: Reading and writing your database

We'll ignore line 11 in the last snippet of Part 1, meaning that we now have
a `project` table that has `id`, `name`, `description`, `due`, and `isDraft`
columns that need modelling into a struct, which is what we'll do be doing in
Part 2, along with some simple reading and writing.
...
succeeded. To get the `id`, you'd need to go back in and read from the database
which is far from ideal. Fortunately, we can tell GRDB to mutate the variable
`insert` was called on once the insertion has completed.

Go back to your model add add the following to the extension:
...
Now, if you were to run the `write` snippet above, `print` would now return the
...
You get the idea, and I hope I've shown just how nice GRDB is to work with.

With the basics out of the way, we'll dig into some of GRDB's more advance
features next week, including database observation and reacting to changes,
saving custom types, and defining associations.
